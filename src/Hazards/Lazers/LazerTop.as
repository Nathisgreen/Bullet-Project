package  Hazards.Lazers
{
	...
	public class LazerTop extends Entity
	{
		...
		public function LazerTop(x:int, y:int, aOnTime:int, aOffTime:int, aConstant:String, delay:int ) 
		{
			constant = aConstant;
			this.x = x;
			this.y = y;
		...
		override public function render():void
		{
			if (done == false && hasLenght == true )
			{
				...
				for each (var i:Point in pointArray)
				{
					if (ii == 0)
					{
						Draw.linePlus(this.x + 8, y + 4, i.x, i.y, 0x770505, 1)
					}
					else
					{
				...
				for each (var ia:Point in pointArraya)
				{
					if (ii == 0) 
					{
						Draw.linePlus(this.x + 8, y + 4, ia.x, ia.y, 0xD10909, 1)
					}
					else
					{
		...
		private function allocateX():void
		{
			for each (var i:Point in pointArray)
			{
				i.x = this.x+8 + randomNumber( -5, 5);
			}
			for each (var ia:Point in pointArraya)
			{
				ia.x = this.x+8 + randomNumber( -5, 5);
			}
		}
		...
		private function autoLength():void
		{
			while (lineLength == 0)
			{
				if (!collide(GC.TYPE_LEVEL, x, y+offset))
				{
					offset += 16;
				}
				else
				{
					...
					{
						bitmapArray[i] = new BitmapData(16, lineLength, true, 0x00000000);
					}
				}	
			}
